...
function add(file){const rel=path.relative(ROOT,file).replaceAll('\\','/');if(skip.has(rel))return;const html=fs.readFileSync(file,'utf8');if(/<meta[^>]+name=["']robots["'][^>]+content=["'][^"']*noindex/i.test(html))return;const m=html.match(/<link[^>]+rel=["']canonical["'][^>]+href=["']([^"']+)["']/i)||html.match(/<link[^>]+href=["']([^"']+)["'][^>]+rel=["']canonical["']/i);let u=m?.[1];if(!u){if(rel==='index.html')u=BASE+'/';else if(rel.endsWith('/index.html'))u=BASE+'/'+rel.slice(0,-10);else u=BASE+'/'+rel}if(u.startsWith(BASE)&&!urls.includes(u))urls.push(u)}
