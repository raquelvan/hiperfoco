...
function replaceSrcInTag(tag, src, alt){
  let next=tag.replace(/\s+srcset=(['"])[\s\S]*?\1/gi,'').replace(/\s+data-src=(['"])[\s\S]*?\1/gi,'');
...
function applyRule(html, rule){
  ...
  while(cursor<out.length){
    ...
    let hit=-1;
    for(const term of rule.terms){const i=low.indexOf(term.toLowerCase(),cursor);if(i!==-1&&(hit===-1||i<hit))hit=i;}
    if(hit===-1) break;
    const prevStart=low.lastIndexOf('<img',hit), prevEnd=prevStart>=0?out.indexOf('>',prevStart):-1;
    const nextStart=low.indexOf('<img',hit), nextEnd=nextStart>=0?out.indexOf('>',nextStart):-1;
    const prevDist=prevStart>=0?hit-prevStart:Infinity, nextDist=nextStart>=0?nextStart-hit:Infinity;
    let start=-1,end=-1;
    if(prevDist<=3200||nextDist<=3200){if(prevDist<=nextDist){start=prevStart;end=prevEnd;}else{start=nextStart;end=nextEnd;}}
    if(start>=0&&end>start){
      ...
      if(!tag.includes(rule.src)){
        ...
        cursor=hit+rule.terms[0].length;continue;
      }
    }
    cursor=hit+1;
  }
...
try{
  ...
  const map={
    'delonghi-magnifica-evo':'/assets/approved/magnifica-evo-v2.jpg',
    'philips-3300':'/assets/approved/philips-3300-v2.jpg',
    'delonghi-rivelia':'/assets/approved/rivelia-v2.jpg',
    'delonghi-magnifica-s':'/assets/approved/magnifica-s-v2.jpg',
    'philips-5500':'/assets/approved/philips-5500-v2.webp'
  };
  for(const [k,src] of Object.entries(map)) if(data[k]) Object.assign(data[k],{image:src,imageLocked:true,imageSource:'approved-hd-local-v2'});
...
console.log(`✓ Fotos aprobadas v2 aplicadas en ${changed} HTML.`);
