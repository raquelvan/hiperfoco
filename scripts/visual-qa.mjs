...
for(const [label,viewport] of sizes){
  ...
  for(const [name,path] of pages){
    ...
    await page.evaluate(async()=>{const imgs=[...document.images];await Promise.all(imgs.map(img=>img.complete?Promise.resolve():new Promise(resolve=>{const done=()=>resolve();img.addEventListener('load',done,{once:true});img.addEventListener('error',done,{once:true});setTimeout(done,5000)})));});
    ...
    const result=await page.evaluate((pageName)=>{
      ...
      if(pageName==='home'){
        ...
        checks.homeReviews=reviews?[...reviews.querySelectorAll('.review-grid>.card')].filter(c=>c.querySelector('.product-media img')).length:0;
