(()=>{
 ...
 const main=document.querySelector('main.article,main.s3-guide');if(!main)return;const simple=main.classList.contains('article');if(simple)main.classList.add('hf-guide');
 ...
 if(simple){
  ...
  if(title&&!main.querySelector('.hf-guide-hero')){const hero=document.createElement('section');hero.className='hf-guide-hero';const left=document.createElement('div');left.className='hf-guide-hero-main';const answer=document.createElement('aside');answer.className='hf-guide-answer';if(eyebrow)left.appendChild(eyebrow);left.appendChild(title);if(lead)left.appendChild(lead);const quick=(lead?.textContent||'Te ayudamos a separar lo importante de las especificaciones que apenas cambian la compra.').split(/(?<=[.!?])\s/)[0];answer.innerHTML='<span>Respuesta rápida</span><strong>'+quick+'</strong><p>Después bajamos a producto, uso real, mantenimiento y alternativas.</p>';hero.append(left,answer);main.prepend(hero)}
  const direct=[...main.children],hs=direct.filter(n=>n.tagName==='H2');if(hs.length&&!main.querySelector('.hf-guide-nav')){const nav=document.createElement('nav');nav.className='hf-guide-nav';hs.slice(0,8).forEach(h=>{h.id=h.id||slug(h.textContent);const a=document.createElement('a');a.href='#'+h.id;a.textContent=h.textContent.replace(/^\d+[.)]?\s*/,'');nav.appendChild(a)});main.querySelector('.hf-guide-hero')?.after(nav)}
  const snapshot=[...main.children].filter(n=>!n.classList?.contains('hf-guide-hero')&&!n.classList?.contains('hf-guide-nav')&&!n.classList?.contains('hf-guide-picks')&&!n.classList?.contains('hf-guide-related'));let section=null;for(const n of snapshot){if(n.tagName==='H2'){section=document.createElement('section');section.className='hf-guide-section';main.insertBefore(section,n);section.appendChild(n)}else if(section&&n.parentElement===main)section.appendChild(n)}
 }
 const pickMap={
